    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                ...
                <form action="{{ route('user.mapel.update', $mapel->id) }}" method="POST" class="card-body">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label class="form-label">Mata Pelajaran</label>
                        <input type="text" value="{{ $mapel->nama ?? old('nama') }}" class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" name="nama" placeholder="ex: Matematika">
                        @if ($errors->has('nama'))
